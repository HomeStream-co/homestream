...
### Improved
- **README complete rewrite of Installation section** — step-by-step first-timer guides for Windows and all Linux install methods (AppImage, .deb, pacman, AUR)
- Windows: SmartScreen bypass instructions, auto-update callout
- Linux AppImage: `wget` one-liner using GitHub API to always fetch the latest release; chmod + run instructions; GUI alternative tip
- Linux .deb: `apt-get install -f` dependency fix tip
- Linux pacman: direct download + install commands
- Added **download file reference table** so users know which file to pick
- Quick Start rewritten as a table with time estimate
- API Keys table now shows whether each key is free or paid
