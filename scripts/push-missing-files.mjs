...
async function pushFile(localPath, remotePath) {
  ...
  await ghPut(remotePath, content, sha, `Sync ${remotePath}`);
}

async function main() {
  // Push airo-secrets files
  const files = [
    ['airo-secrets/src/index.ts', 'airo-secrets/src/index.ts'],
    ['airo-secrets/src/secrets-utils.ts', 'airo-secrets/src/secrets-utils.ts'],
  ];

  for (const [local, remote] of files) {
    if (fs.existsSync(local)) {
      await pushFile(local, remote);
    } else {
      console.log('MISSING locally:', local);
    }
  }

  // Check if airo-secrets/package.json exists
  if (fs.existsSync('airo-secrets/package.json')) {
    await pushFile('airo-secrets/package.json', 'airo-secrets/package.json');
  }
}
