/**
 * electron-info.test.ts — GET /api/electron
 *
 * Tests the Electron .exe ↔ Express bridge endpoint.
 * Covers: platform detection, OS-derived media dirs, isElectron flag,
 * Electron-injected dir override, response shape, .exe-specific scenarios.
 */
...
const KEYS = ['HOMESTREAM_PLATFORM', 'HOMESTREAM_DEFAULT_MEDIA_DIR', 'ELECTRON'];

function saveEnv()    { for (const k of KEYS) SAVED[k] = process.env[k]; }
function restoreEnv() {
  for (const k of KEYS) {
...
describe('GET /api/electron — OS-derived media dir (no env var)', () => {
  ...
  it('unknown OS → falls back to linux-style path containing HomeStream', () => {
...
describe('GET /api/electron — isElectron flag', () => {
  ...
  it('isElectron: true when ELECTRON="1"', () => {
  ...
  it('isElectron: true when ELECTRON="true"', () => {
  ...
  it('isElectron: false when ELECTRON is absent', () => {
  ...
  it('isElectron: false in cloud/dev mode', () => {
    delete process.env.ELECTRON;
    delete process.env.HOMESTREAM_PLATFORM;
    const res = makeRes();
    handler(makeReq(), res as unknown as Response);
...
describe('GET /api/electron — response shape', () => {
  ...
  it('defaultMediaDir always ends with HomeStream for all platforms', () => {
    delete process.env.HOMESTREAM_DEFAULT_MEDIA_DIR;
