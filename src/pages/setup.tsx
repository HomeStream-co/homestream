  ...
  const completeSetup = async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        toast.success('HomeStream is ready!');
          ...
          >
            ...
            {step === 5 && (
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2 text-sm">
                  {[
                    ...
                    <div key={label} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                      ...
                      <span className={`flex items-center gap-1.5 font-medium ${ok ? 'text-foreground' : 'text-muted-foreground'}`}>
                        {ok ? <CheckCircle2 className="w-3.5 h-3.5 text-green-400" /> : <AlertCircle className="w-3.5 h-3.5 text-muted-foreground" />}
                        {value}
                      </span>
                    </div>
                  ))}
                ...
                <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 text-xs text-muted-foreground leading-relaxed">
                  <strong className="text-foreground">Next step:</strong> Open Jellyfin at <a href={form.jellyfinUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{form.jellyfinUrl}</a> and add <code className="bg-muted px-1 rounded">{form.mediaDir}/library</code> as a media library. HomeStream will auto-populate it as you download content.
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={completeSetup} disabled={status.complete === 'saving' || status.complete === 'done'}
                    ...
                    {status.complete === 'saving' ? <><Loader2 className="w-4 h-4 animate-spin" />Starting HomeStream…</> :
                     status.complete === 'done' ? <><CheckCircle2 className="w-4 h-4" />Done! Redirecting…</> :
