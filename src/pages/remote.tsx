/**
 * /remote — Phone Remote Control  (v2)
 *
 * Mobile-optimised WebSocket remote for HomeStream.
 *
 * New in v2:
 *  - Poster art backdrop with blur + gradient overlay
 *  - Subtitle / caption track toggle (cycles through available tracks)
 *  - Haptic feedback on every button press (navigator.vibrate)
 *  - Swipe gestures on the poster area:
 *      horizontal swipe → seek ±10s (or ±30s for fast swipe)
 *      vertical swipe   → volume up/down
 *  - Landscape layout: poster left, controls right
 *  - Fullscreen toggle command
 *  - Cast button (sends 'cast' command — handled by CastButton on player)
 */
  ...
  return (
    <div className={`min-h-screen bg-background select-none overflow-hidden relative ${isLandscape ? 'flex flex-row' : 'flex flex-col items-center'}`}>
      ...
      {isLandscape && status === 'connected' && state ? (
        ...
        ════════════════════════════════════════════════════════════════════ */
        <div className="w-full max-w-sm mx-auto flex flex-col px-4 pt-5 pb-10">
