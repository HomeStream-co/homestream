 ...
 * Movies Page — /movies
 *
 * Filtered browse view showing only movies (type === 'movie').
 * Clicking a card navigates to /movie/:id for the full detail view.
 */
...
export default function MoviesPage() {
  ...
  const filtered = useMemo(() => {
    // Movies only + parental filter
    let items = library.filter(m => m.type === 'movie' && isAllowed(m.rated));
  ...
  return (
    <div className="bg-background pt-20 pb-16">
      <title>Movies — HomeStream</title>
      <meta name="description" content="Browse your movie collection on HomeStream." />
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-4xl font-heading text-foreground">Movies</h1>
        </div>
        ...
        <p className="text-sm text-muted-foreground mb-4">
          {loading ? 'Loading...' : `${filtered.length} movie${filtered.length !== 1 ? 's' : ''}`}
        </p>
