...
export default function VPNPanel() {
  ...
  useEffect(() => {
    fetchStatus();
    // Poll status every 10s while panel is visible
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {!isConfigured
                ...
                : isConnected
                  ...
                  : `${config ? providers.find(p => p.id === config.provider)?.name ?? config.provider : 'VPN'} · Disconnected`
              }
            </p>
          </div>
        </div>
              ...
              {isConfigured && (
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  {(status?.activeDownloads ?? 0) > 0 && (
                    <div className="p-2.5 bg-green-500/10 border border-green-500/20 rounded-lg col-span-2">
                        ...
                        {providers.map(p => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setEditProtocol(p.protocol as VPNProtocol);
                            }}
                        ...
                        <label className="absolute bottom-2 right-2 cursor-pointer">
                          <input type="file" accept=".conf,.ovpn,.txt" className="hidden"
                            onChange={async e => {
                              ...
                              if (file) setEditConfig(await file.text());
                            }}
                          />
