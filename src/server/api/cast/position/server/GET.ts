 ...
 * endpoint reads from in-memory state that is updated every 5 s by the server.
 * It is safe to call frequently from any client without hammering the TV.
 *
 * Returns:
 *   { ok: true,  currentTime, duration, mediaId, updatedAt }  — position available
 *   { ok: false, error: 'No active DLNA cast' }               — nothing is casting
 */
